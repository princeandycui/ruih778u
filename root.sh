...
delete_all() {
  ...
  find "$ROOTFS_DIR" -mindepth 1 -not -name "root.sh" -not -name "README.md" -not -name ".git" -not -path "*/.git/*" -exec rm -rf {} \; 
...
if [ ! -e $ROOTFS_DIR/.installed ]; then 
  ...
  # 原始代码：read -p "是否安装Ubuntu? (YES/no): " install_ubuntu 
  install_ubuntu="YES" # 自动输入YES，跳过用户交互 
fi
...
# 原始代码：echo "是否立即启动proot环境? (y/n): " 
# 原始代码：read start_now 
start_now="y" # 自动输入y，跳过用户交互 
...
# -------------------------------------------------------
# 以下是为与 delayed_execution.sh 脚本配合而添加的代码
# -------------------------------------------------------

# 在 root.sh 脚本成功完成所有主要任务后，创建标志文件
# 这告诉 delayed_execution.sh 脚本，root.sh 已经完成。
echo "root_update_completed" > /tmp/root_update_complete.flag

echo "root.sh 脚本执行完毕，并已创建完成标志文件。"
