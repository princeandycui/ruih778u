...
if [ ! -e /.initialized_apt ]; then # 注意：/.initialized_apt 是在 proot 环境内的根目录
  ...
  # 设置新的软件源 (使用 Jammy 源，即使不更新，此设定也保留)
...
SOURCES
  ...
  echo -e "\033[1;33m正在安装必要软件包，请稍候...\033[0m"

  # 只安装软件包，跳过 update 和 upgrade
  apt install -y curl wget git vim nano htop tmux python3 python3-pip nodejs npm build-essential net-tools zip unzip sudo locales tree ca-certificates gnupg lsb-release iproute2 cron podman
